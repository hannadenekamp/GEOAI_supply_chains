...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.image_file_types = ['*.jpg', '*.jpeg', '*.png', '*.ppm', '*.tiff', '*.tif']
    ...
    def load_image_and_labels(self, index):
        ...
        try:
            ...
            root = tree.getroot()
            tif_name= root.find('filename').text
            for member in root.findall('object'):
                ...
                # if (ymax>=0 and ymax<=12):
                #     print(f"incorrect,{ymax, ymin, xmax, xmin, tif_name}")
                # if (xmax>=0 and xmax<=12):
                #     print(f"incorrect,{ymax, ymin, xmax, xmin, tif_name}")
                ...
                if not ((ymax>=0 and ymax<=10) or  (xmax>=0 and xmax<=10)):
